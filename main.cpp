...
void printMetrics(const AvoidingKuratowskiGraphsPainter::MetricsMap& metrics, std::ostream& out) {
  ...
  for (const auto& outerPair : metrics) {
    ...
    out << std::setw(5) << outerKey;
    for (size_t col : columnKeys) {
      auto it = outerPair.second.find(col);
      if (it != outerPair.second.end()) {
        out << std::setw(5) << "(" << std::fixed << std::setprecision(2) << it->second.k33 << ", " << it->second.k5 << ")";
      }
      else {
        out << std::setw(5) << "(" << std::fixed << std::setprecision(2) << 0.0F << ", " << 0.0F << ")";
      }
    }
    out << std::endl;
  }
...
int main() {
  ...
  const BAGraph graph1 = random_graph_factory.createBarabasiAlbertWithPreferentialAttachmentBatageljBrandes(10, 30, 5, &painter);
