...
def expected_number_of_index_choices_lower_bound(
    ...
    """
    Calculates the lower bound of expected number of ways to choose indices for a specific subgraph within a larger graph.
    ...
    return sympy.Integer(1) / sympy.factorial(subgraph_vertices_number) ** (
        sympy.Integer(subgraph_edges_number) / sympy.Integer(subgraph_vertices_number)
    )


def expected_number_of_index_choices_upper_bound(
    graph_vertices_number: Union[sympy.Integer, sympy.Symbol],
    graph_average_degree: Union[sympy.Integer, sympy.Symbol],
    subgraph_vertices_number: int,
    subgraph_edges_number: int,
...
def expected_number_of_subgraphs(
    ...
    Parameters:
        ...
        string mode: 'lower_bound' or 'upper_bound'  to specify the calculation mode.
    ...
    if mode == "lower_bound":
        return total_probability * expected_number_of_index_choices_lower_bound(
            graph_vertices_number,
        ...
        return total_probability * expected_number_of_index_choices_upper_bound(
            graph_vertices_number,
            graph_average_degree,
            subgraph_vertices_number,
            subgraph_edges_number,
        ...
        raise ValueError("Mode must be either 'lower_bound' or 'upper_bound'")
...
if __name__ == "__main__":
    expected_number_of_k3_subgraphs_lower = expected_number_of_cycle_subgraphs(
        sympy.symbols("n"), sympy.symbols("m"), 3, "lower_bound"
    )
    expected_number_of_k3_subgraphs_upper = expected_number_of_cycle_subgraphs(
        sympy.symbols("n"), sympy.symbols("m"), 3, "upper_bound"
    )
    print(f"Expected number of K3 subgraphs >= {expected_number_of_k3_subgraphs_lower}")
    print(f"Expected number of K3 subgraphs <= {expected_number_of_k3_subgraphs_upper}")

    expected_number_of_k5_subgraphs_lower = expected_number_of_clique_subgraphs(
        sympy.symbols("n"), sympy.symbols("m"), 5, "lower_bound"
    )
    expected_number_of_k5_subgraphs_upper = expected_number_of_clique_subgraphs(
        sympy.symbols("n"), sympy.symbols("m"), 5, "upper_bound"
    )
    print(f"Expected number of K5 subgraphs >= {expected_number_of_k5_subgraphs_lower}")
    print(f"Expected number of K5 subgraphs <= {expected_number_of_k5_subgraphs_upper}")

    expected_number_of_k33_subgraphs_lower = (
        expected_number_of_bipartite_complete_subgraphs(
            sympy.symbols("n"), sympy.symbols("m"), 3, "lower_bound"
        )
    )
    expected_number_of_k33_subgraphs_upper = (
        expected_number_of_bipartite_complete_subgraphs(
            sympy.symbols("n"), sympy.symbols("m"), 3, "upper_bound"
        )
    )
    print(
        f"Expected number of K3,3 subgraphs >= {expected_number_of_k33_subgraphs_lower}"
    )
    print(
        f"Expected number of K3,3 subgraphs <= {expected_number_of_k33_subgraphs_upper}"
    )
