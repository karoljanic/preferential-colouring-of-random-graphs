...
class GraphPainter {
 public:
  ...
  GraphPainter(std::vector<ColorType> nodes_colors, std::vector<ColorType> edges_colors)
  ...
  [[nodiscard]] inline size_t getNodesColorsNumber() const { return nodes_colors_.size(); }
  [[nodiscard]] inline size_t getEdgesColorsNumber() const { return edges_colors_.size(); }
 ...
 protected:
  std::vector<ColorType> nodes_colors_;
  std::vector<ColorType> edges_colors_;
};
}  // namespace graph::random
